...
def evaluate(board):
    ...
    for i in range(8):
        for j in range(8):
            piece_color = board[i][j]
            if piece_color == 0:  # 空位，跳过
                continue
            score = 0
            if (i == 0 and j == 0) or (i == 0 and j == 7) or (i == 7 and j == 0) or (i == 7 and j == 7):
                ...
                score = 1
            # 由于评估函数只在AI回合中使用，所以只需判断是AI还是对手的棋子
            if piece_color == -1:  # AI的棋子
                total_score += score
            else:  # 对手的棋子
                total_score -= score
    return total_score
        ...
        for i in range(8):
            for j in range(8):
                if is_valid_move(board, i, j, color):
                    ...
                    current_score, _ = alphabeta(temp_board, depth - 1, alpha, beta, -color)
...
def main():
    ...
    while running:
        ...
        if not any(is_valid_move(board, row, col, current_player) for row in range(BOARD_SIZE) for col in
            ...
            if not any(is_valid_move(board, row, col, current_player) for row in range(BOARD_SIZE) for col in
                ...
                running = False  # 设置running为False，表示游戏结束
        ...
        if running:
            ...
            if pos is None:
                ...
                _, move = alphabeta(board, 6, float('-inf'), float('inf'), current_player)
                if move:
    ...
    while not running:  # 当游戏结束时，这个循环会保持窗口打开，直到用户关闭窗口
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = True  # 用户关闭窗口时，设置running为True，跳出循环
    pygame.quit()
